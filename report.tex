...
where we have introduced the variable $V_0 = \frac{nk_BT}{|q|}$. Here $T$ is the surrounding temperature and $n$ the ideality factor. Using non-linear least squares optimization for the observed data from the measurement without illumination, we obtained the best-fit values $I_{sat}=1.78\cdot 10^{-6}$ A and $V_0=0.0498$ V. See figure $\ref{ivsun}$, which shows the congruence between theory and experiment, but we also see slight systematic deviation for positive voltage due to small currents in the backward bias. \\

We found the ideality factor to be $n = \frac{|q|V_0}{k_BT} = 1.93$. This can be compared with an ideal diode, which should have the value $1$. The occurence of recombination of electrons and holes could account for some of the increase.
electrons. We finally note that there was a current even at zero bias. This is because ambient light was present, which made the solar cell produce electricity. \\
